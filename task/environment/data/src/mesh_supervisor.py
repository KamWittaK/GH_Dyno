...
class Supervisor:
    ...
    def safe_gen(self,src):
        live=[g for g in self.gens.values() if g.service==src.service and g.state!='exited']
        return max(live,key=lambda g:g.generation_id) if live else None
    ...
    def read_worker(self,src,fileobj):
        ...
        while b'\n' in g.buffer:
            ...
            if typ=='ready':
                ...
                if g.transaction is None:
                    self.commit_initials()
                else:
                    self.commit(g)
                    self.finish_rollout_if_ready()
            elif typ=='failed':
    ...
    def perform_reexec_checkpoint(self):
        ...
        checkpoint_now=monotonic()
        for gid,d in restored['deadlines'].items():
            g=self.gens.get(int(gid))
            if not g:continue
            spec=self.services[g.service]
            g.startup_deadline=(checkpoint_now+spec.get('startup_timeout_ms',1000)/1000) if d['startup'] else None
            g.runtime_deadline=(checkpoint_now+spec.get('runtime_timeout_ms',0)/1000) if d['runtime'] else None
            g.kill_deadline=(checkpoint_now+spec.get('grace_ms',120)/1000) if d['kill'] else None
    ...
    def done(self):
        if self.shutting and not any(g.state!='exited' for g in self.gens.values()):return True
